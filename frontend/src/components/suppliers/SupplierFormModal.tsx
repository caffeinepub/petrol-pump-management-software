...
import { useAppStore, Supplier, FuelType } from '../../store/appStore';
  ...
  useEffect(() => {
    if (open) {
      if (editingSupplier) {
        ...
        setFuelTypes(editingSupplier.fuelTypes ?? editingSupplier.fuelTypesSupplied ?? []);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingSupplier) {
      updateSupplier(editingSupplier.id, {
        ...
        rating: parseFloat(rating) || editingSupplier.rating,
      });
    } else {
      ...
      <DialogContent className="max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-1">
            ...
            <Input id="name" value={name} onChange={e => setName(e.target.value)} placeholder="Company name" required />
          </div>
          ...
          <div className="space-y-1">
            ...
            <Input id="address" value={address} onChange={e => setAddress(e.target.value)} placeholder="City, State" />
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-3">
              {ALL_FUEL_TYPES.map(ft => (
                <label key={ft} className="flex items-center gap-1.5 cursor-pointer text-sm">
                  <Checkbox
          ...
          <div className="space-y-1">
            ...
            <Input id="rating" type="number" min={1} max={5} step={0.1} value={rating} onChange={e => setRating(e.target.value)} placeholder="e.g. 4.5" />
          </div>
