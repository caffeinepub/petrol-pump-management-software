...
export default function MetricCard({ title, value, unit, icon: Icon, trend, statusColor = 'amber', subtitle }: MetricCardProps) {
  return (
    <div className={`bg-navy-700 rounded-2xl p-5 border ${statusColors[statusColor]} shadow-card`}>
      <div className="flex items-start justify-between mb-3">
        <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${iconColors[statusColor]}`}>
          <Icon className="w-5 h-5" />
        </div>
        {trend && (
          <span className={`text-xs font-medium px-2 py-1 rounded-full ${trend.value >= 0 ? 'bg-fuel-petrol/20 text-fuel-petrol' : 'bg-destructive/20 text-destructive'}`}>
