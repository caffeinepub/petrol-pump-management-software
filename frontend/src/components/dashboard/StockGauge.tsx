...
const fuelColors: Record<FuelType, { bar: string; text: string; bg: string }> = {
  Petrol: { bar: 'bg-fuel-petrol', text: 'text-fuel-petrol', bg: 'bg-fuel-petrol/10' },
  Diesel: { bar: 'bg-fuel-diesel', text: 'text-fuel-diesel', bg: 'bg-fuel-diesel/10' },
  Premium: { bar: 'bg-amber', text: 'text-amber', bg: 'bg-amber/10' },
};

export default function StockGauge({ fuelType, currentLevel, capacity, threshold }: StockGaugeProps) {
  ...
  const statusColor = isCritical ? 'text-destructive bg-destructive/20' : isLow ? 'text-amber bg-amber/20' : 'text-fuel-petrol bg-fuel-petrol/20';
  const barColor = isCritical ? 'bg-destructive' : isLow ? 'bg-amber' : colors.bar;

  return (
    <div className={`bg-navy-700 rounded-2xl p-5 border ${isLow ? 'border-amber/40' : 'border-navy-600'} shadow-card`}>
      <div className="flex items-center justify-between mb-3">
        <div>
          <h3 className={`font-display text-lg ${colors.text}`}>{fuelType}</h3>
          <p className="text-xs text-muted-foreground">Tank Capacity: {capacity.toLocaleString()}L</p>
        </div>
      ...
      <div className="flex justify-between text-sm">
        <div>
          <span className={`font-semibold ${colors.text}`}>{currentLevel.toLocaleString()}L</span>
      ...
      <p className="text-xs text-muted-foreground mt-1">
        Threshold: <span className="text-amber">{threshold.toLocaleString()}L</span>
      </p>
    </div>
  );
}
