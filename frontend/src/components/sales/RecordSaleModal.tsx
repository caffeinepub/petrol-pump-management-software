...
import { useAppStore, FuelType, PaymentMethod, FuelSale } from '../../store/appStore';
...
export default function RecordSaleModal({ open, onClose }: Props) {
  ...
  const stockOk = selectedInventory ? litresNum <= selectedInventory.currentStock : true;

  const validate = () => {
    const errs: Record<string, string> = {};
    if (!fuelType) errs.fuelType = 'Select a fuel type';
    if (!litres || litresNum <= 0) errs.litres = 'Enter valid litres';
    if (!pricePerLitre || priceNum <= 0) errs.pricePerLitre = 'Enter valid price';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (selectedInventory) {
        adjustStock(selectedInventory.id, -litresNum, 'Sale', staffMember?.name ?? recordedBy);
      }

      toast.success(`Sale recorded: ${litresNum}L ${fuelType} for ₹${total.toFixed(2)}`);
      onClose();
    } catch {
      ...
      <DialogContent className="max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-1">
            ...
            <Select value={fuelType} onValueChange={v => { setFuelType(v as FuelType); setErrors(p => ({ ...p, fuelType: '' })); }}>
              <SelectTrigger>
            ...
            {selectedInventory && (
              <p className={`text-xs ${stockOk ? 'text-muted-foreground' : 'text-destructive'}`}>
          ...
          <div className="space-y-1">
            ...
            <Input
              ...
              step={0.01}
              value={litres}
              onChange={e => { setLitres(e.target.value); setErrors(p => ({ ...p, litres: '' })); }}
              placeholder="e.g. 20"
            />
            {errors.litres && <p className="text-destructive text-xs">{errors.litres}</p>}
          </div>
          ...
          <div className="space-y-1">
            ...
            <Input
              ...
              step={0.01}
              value={pricePerLitre}
              onChange={e => { setPricePerLitre(e.target.value); setErrors(p => ({ ...p, pricePerLitre: '' })); }}
          ...
          <div className="space-y-1">
            ...
            <Select value={paymentMethod} onValueChange={v => { setPaymentMethod(v as PaymentMethod); setErrors(p => ({ ...p, paymentMethod: '' })); }}>
              <SelectTrigger>
          ...
          <div className="space-y-1">
            ...
            <Input
              ...
              min={1}
              value={pumpNumber}
              onChange={e => { setPumpNumber(e.target.value); setErrors(p => ({ ...p, pumpNumber: '' })); }}
              placeholder="e.g. 1"
            />
          ...
          <div className="space-y-1">
            ...
            <Select value={recordedBy} onValueChange={v => { setRecordedBy(v); setErrors(p => ({ ...p, recordedBy: '' })); }}>
              <SelectTrigger>
            ...
            {errors.recordedBy && <p className="text-destructive text-xs">{errors.recordedBy}</p>}
          </div>
