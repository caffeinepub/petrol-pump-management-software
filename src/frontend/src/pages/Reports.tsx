...
export default function Reports() {
  ...
  const dailySummary = useMemo(() => {
    ...
    return fuelTypes.map((ft) => {
      ...
      return {
        ...
        volume,
        revenue,
        avgPrice: volume > 0 ? revenue / volume : 0,
      };
    });
  ...
  const expenseVsRevenue = useMemo(() => {
    const filteredSales = fuelSales.filter(
      (s) => s.date >= expenseStartDate && s.date <= expenseEndDate,
    );
    const filteredExpenses = expenses.filter(
      (e) => e.date >= expenseStartDate && e.date <= expenseEndDate,
    );
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border">
                <th className="text-left text-muted-foreground py-2 pr-4 whitespace-nowrap">
                  Fuel Type
                </th>
                <th className="text-right text-muted-foreground py-2 px-4 whitespace-nowrap">
                  Volume (L)
                </th>
                <th className="text-right text-muted-foreground py-2 px-4 whitespace-nowrap">
                  Revenue (&#8377;)
              ...
              {dailySummary.map((row) => (
                ...
                >
                  <td
                    ...
                    style={{ color: FUEL_COLORS[row.fuelType] ?? "#888" }}
                  >
                    {row.fuelType}
                  </td>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 gap-2">
            ...
            <div className="flex gap-2 flex-wrap">
              <Input
                ...
                className="w-36 text-xs min-h-[44px]"
              />
              <Input
                type="date"
